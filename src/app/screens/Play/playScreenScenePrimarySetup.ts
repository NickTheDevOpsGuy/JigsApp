...
import { usePlayScreenSceneState } from "./hooks/usePlayScreenSceneState";
import { usePlayScreenManager } from "./hooks/usePlayScreenManager";
import { buildPlayScreenManagerConfig } from "./playScreenManagerConfig";
import { getQuadrantPb, setQuadrantPb } from "./timeMode";
import { parseGrid, STORAGE_KEY, GRID_KEY, GRID_ONCE_KEY } from "./playScreenUtils";
...
export function usePlayScreenPrimarySetup() {
  ...
  const localImageUrl = useMemo(() => safeLocalStorage.getItem(STORAGE_KEY) ?? "", [dailyLinkApplied]);
  ...
  const { timeMode, setTimeMode, countdownMinutes, setCountdownMinutes } = useTimeModeConfig();
  ...
  const managerConfig = buildPlayScreenManagerConfig({
    ...
    onPrecisionSnap: ui.precisionModeEnabled
      ? (precisionPx) => {
          scene.precisionSnapsRef.current = [...scene.precisionSnapsRef.current, precisionPx];
        }
      : undefined,
