...
export function updatePerfStats(
  ...
  const activeGroups = new Set(st.pieces.filter((p) => !p.inTray).map((p) => p.groupId))
    .size;
...
export function buildLockLerpOverrides(args: {
  ...
  if (!isDragging && !reducedMotion) {
    ...
    for (const p of st.pieces) {
      ...
      /* Quintic ease-out — softer deceleration than cubic (less abrupt “snap into lock”) */
      const ease = 1 - (1 - t) ** 5;
      const liftPx = Math.min(LOCK_LIFT_MAX_PX, Math.max(3, p.h * 0.045));
      const liftOffset = liftPx * (1 - t) ** 3;
