...
import type {
  ...
  DragPreviewState,
  ScreenToBoard,
} from "@/screens/Play/hooks/pointerHandlers/types";
import { resetTouchState } from "@/screens/Play/hooks/pointerHandlers/touchHandlers";
import { createPointerHandlers } from "@/screens/Play/hooks/pointerHandlers/pointerHandlersFactory";
...
export function canRotateBoardPieceInState(st: PuzzleState, pid: PieceId): boolean {
  ...
  return !groupPieces.some((p) => p.locked);
}
...
export function usePointerHandlers(args: {
  ...
  const lastTouchDragEndTimeRef = useRef<number>(0);
  ...
  }, [manager, onDragPreview, setState, onDragEnded]);

  // Touch-event-based pinch zoom for iOS Safari (pointer events are unreliable for multi-touch).
  useEffect(() => {
