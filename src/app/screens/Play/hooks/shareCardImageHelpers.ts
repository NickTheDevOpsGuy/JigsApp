...
export function roundedRectPath(ctx: CanvasRenderingContext2D, rect: Rect, radius: number) {
...
export function drawCardBackground(ctx: CanvasRenderingContext2D, rect: Rect, radius: number) {
  const bg1 = "#0a0e1a";
  const bg2 = "#1a1f35";
...
export function drawPuzzleIcon(ctx: CanvasRenderingContext2D, x: number, y: number, size: number) {
  const w = size;
  const h = size * 1.05;
  const r = size * 0.18;
  ctx.save();
