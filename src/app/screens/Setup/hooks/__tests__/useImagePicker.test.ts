...
describe("fetchImageUrlAsDataUrl", () => {
  ...
  it("rejects invalid URLs before fetching", async () => {
    await expect(fetchImageUrlAsDataUrl("not-a-url", vi.fn() as typeof fetch)).rejects.toThrow(
  ...
  it("rejects non-image responses", async () => {
    ...
    ).rejects.toThrow("That URL did not return an image. Try a direct PNG, JPG, or WebP link.");
  });
