import type { DragPreview, DragState, Piece, PuzzleState } from "./types";
import type { MutableRefObject } from "react";
import type { PuzzleManagerEvents, PuzzleManagerOptions } from "./puzzleManagerTypes";
import { createInitialPieces } from "./factories/createInitialPieces";
import { UndoManager } from "./undoManager";
import { getUndoLimit, type EffectiveToleranceOptions } from "./puzzleManagerUtils";
import {
...
} from "./puzzleManagerDerivedState";
import {
...
} from "./puzzleManagerGroupHelpers";
...
export class PuzzleManagerState {
  ...
  protected recomputeDerivedState() {
    ...
    if (!prevComplete && isComplete) {
      ...
      this.updatePieces(() => true, () => ({ isPlaced: true }));
      this.events.onPuzzleComplete?.(this.state);
    }
  }

  protected updatePieces(predicate: (p: Piece) => boolean, updater: (p: Piece) => Partial<Piece>) {
    this.state = {
      ...this.state,
  ...
  protected clampGroupInsideBoardInterior(groupId: string) {
    ...
    if (dx !== 0 || dy !== 0) {
      this.updatePieces((p) => p.groupId === groupId, (p) => ({ x: p.x + dx, y: p.y + dy }));
    }
  }
  ...
  protected wouldOverlapAnyOtherGroup(
    ...
    return wouldOverlapUtil(this.state.pieces, groupId, dx, dy, overlapEpsilonPx, ignoreGroupIds);
  }
