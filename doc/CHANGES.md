...
- **Puzzle logic** – `puzzle/PuzzleManager.ts` (drag, snap, groups, undo); helpers in `puzzle/puzzleManagerUtils.ts` (clamp, getUndoLimit, getEffectiveTolerance), `puzzleManagerRestore.ts` (applySavedPieces for undo/redo). Board drawing in `puzzle/canvas/`: `renderBoardDraw.ts`, `renderBoardDrawPiece.ts`, `renderBoardDrawPieceHelpers.ts` (stroke, cached piece), `renderBoardDrawOverlays.ts` (wrong-rotation, lock glow).
