...
2. If yesterday was not completed and no freeze was used for it, and the user has a freeze available, the modal shows an offer: _"You missed yesterday. Use your streak freeze to protect your streak?"_
3. **Use Freeze** – Consumes the token and marks yesterday as "freeze used"; streak continues.
...
- `refreshStreakFreeze()` – Refills to 1 when week changes.
...
`initStreakFreeze()` is called at app startup in `App.tsx` to refresh the weekly token.
